...
- **실제 학습 검증 완료 (2026-08-26)**: 학습 이미지를 GitHub Actions로 빌드해 GHCR에 올리고(`ghcr.io/gjooo/unwork-sd15-lora`), 실제 RTX 4090에서 SD 1.5 LoRA 50 step을 완주했다. 준비 7분 30초 + 학습 7초 + 종료 확인. 실제 실행에는 `MVP_MAX_RUNTIME_MINUTES=15`가 필요하다
- 남은 작업: 프런트엔드 화면 연동, 배포 환경 확정
