...
랜딩에서 요금 모델을 설명하되, **데모에서는 청구하지 않는다는 것을 명시한다.**

숫자를 다룰 때 지켜야 할 사실은 세 가지다.

1. **앱이 내려주는 `estimatedGpuCostKrw`(450 / 650 / 900원)에는 수수료가 들어 있지 않다.** GPU 사용료 추정치일 뿐이다. 백엔드는 수수료를 계산하지도 응답에 담지도 않는다.
2. **BYOK 이후 GPU 비용은 우리를 거치지 않는다.** 사용자가 연결한 본인 Runpod 계정에서 직접 발생한다. 우리가 GPU 사용료를 청구하는 구조가 아니다.
3. `PricingPolicy(percentage_rate=0.15)`는 레거시 optimizer 코드에만 있고 MVP 실행 경로에 적용되지 않는다.

그래서 랜딩 문구는 "향후 과금 모델"로 쓰고 현재 청구하지 않음을 함께 적는다. 예시:

> GPU 사용료는 연결하신 본인 계정에서 직접 발생합니다. Agent 실행 수수료는 향후 GPU 사용료의 15%로 계획하고 있으며, 지금은 청구하지 않습니다.

앱 화면의 예상 비용 옆에 수수료를 더한 합계를 만들어 보여주지 않는다. 백엔드가 그 값을 내려주지 않으므로 화면에서 계산하면 근거 없는 숫자가 된다.
