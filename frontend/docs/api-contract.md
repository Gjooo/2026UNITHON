# 프런트엔드가 필요로 하는 API 계약 (초안)

> **이 문서는 합의된 계약이 아니다.** 프런트엔드가 [PRD-final.md](../../PRD-final.md)의 제품 기능에서 도출한 **요구사항 초안**이며, 화면을 만들고 MSW fixture를 세우기 위한 프런트 내부 작업 문서다.
>
> 합의된 서버 계약은 저장소 루트의 [API-spec.md](../../API-spec.md)이고, 그 문서의 소유권은 백엔드에 있다. 프런트엔드는 그 문서를 고치지 않는다. 이 초안과 `API-spec.md`가 어긋나는 부분은 백엔드 담당자에게 delta로 전달해 합의한 뒤, 합의된 내용만 `API-spec.md`에 반영한다.
>
> 합의 전까지 프런트엔드는 이 초안을 MSW fixture로만 사용하고, 실제 backend에 붙이지 않는다.
