...
**실제 GPU에서 검증 완료 (2026-08-26, RTX 4090)** — 이미지를 빌드해 GHCR에 올리고 실제 Pod에서 학습을 돌렸다.

```text
step 30/50 loss=0.0015 elapsed=4s
step 40/50 loss=0.0024 elapsed=6s
step 50/50 loss=0.0095 elapsed=7s
LoRA 가중치를 저장했습니다: /workspace/output
Training completed. 50/50 steps.
```

승인부터 완료까지 455초가 걸렸고, 그중 **학습은 7초**였다. 나머지 약 7분 반은 이미지 pull과 모델 로딩이다. 이 비율이 이 컨테이너의 가장 중요한 사실이다.

| 구간 | 시간 |
| --- | --- |
| 이미지 pull + 컨테이너 기동 + 모델 로딩 | 약 7분 30초 |
| 학습 50 step | 7초 (초당 약 7 step) |
| 삭제와 종료 확인 | 약 1초 |

### 여기서 나오는 결론

- **10분 상한은 지금 이미지로는 빠듯하다.** 준비에만 7분 반이 들어 학습에 쓸 시간이 2분 남짓이다.
- 학습 자체는 매우 빠르다. 초당 7 step이면 2000 step도 5분이 안 된다. 시간을 잡아먹는 쪽은 학습이 아니라 준비다.
### `runpod/pytorch` 베이스로 바꾸는 안은 측정해 보고 접었다

Runpod이 자기 베이스 이미지를 머신에 캐시해 두었다면 pull 양이 줄 것이라 보고, 그 베이스만으로 Pod를 띄워 컨테이너가 뜨는 시각을 쟀다.

```text
runpod/pytorch:2.4.0-py3.11-cuda12.4.1-devel-ubuntu22.04
→ 537초가 지나도 컨테이너가 뜨지 않음 (지금 이미지는 약 450초)
```

캐시돼 있지 않았고, 그 베이스는 압축 7.43GB로 지금 쓰는 `pytorch/pytorch` 런타임(3.93GB)의 두 배다. 바꿨으면 더 느려졌을 것이다.

여기서 알 수 있는 것은 베이스 선택이 문제가 아니라 **Runpod 머신의 image pull 자체가 느리다**는 사실이다. 실측 약 13MB/s였고, 이 속도에서는 이미지 크기가 그대로 준비 시간이 된다.

### 그래서 택한 방향

- 실제 GPU 실행에는 `MVP_MAX_RUNTIME_MINUTES`를 15로 둔다. 코드 변경 없이 환경변수로 해결된다.
- 부스 시연은 시뮬레이터로 진행한다. 실제 GPU 실행은 "실제로 된다"는 근거로 쓰고, 관람객을 8분 세워두지 않는다.
- 이미지를 더 줄이려면 가중치를 굽지 않고 실행 시점에 받는 방법이 있다(압축 1.9GB 감소). 다만 데모 중 네트워크에 의존하게 되므로 아직 택하지 않았다.
